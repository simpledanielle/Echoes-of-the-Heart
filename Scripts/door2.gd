...
var required_emotion = Emotion.ANGRY  # Set this to the specific emotion for the door
var next_scene_path = "res://Scenes/angrypuzzle.tscn"  # Path to the next scene
	...
	if player_in_range and Input.is_action_just_pressed("ui_interact"):
		...
		if players.size() > 0:
			...
			if player.current_emotion == required_emotion:
				...
				get_tree().change_scene_to_file("res://Scenes/angrypuzzle.tscn")
			else:
